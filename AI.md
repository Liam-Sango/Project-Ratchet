...
3. **Commit `77d722531a5a55f6484f770f5d0296e7a547546e` — Fixture event instrumentation (Phase B1)**  
   An LLM generated the bulk of the event-emit insertions across `keys.py`, `crypto_wrapper.py`, `stego.py`, and `vm.py`. The work was delegated because it is highly repetitive boilerplate: each insertion follows an identical shape (a module-level `emit` import, then `emit(EventType.X, {detail})` at a mapped point, with a success/fail split inside existing `try`/`except` branches) repeated across four files and approximately fifteen insertion points. All non-boilerplate design was specified manually upstream in a per-file instrumentation map — including the `_h16` key-fingerprint helper (to avoid logging raw key material), placement of `VM_END` in the `try` block rather than `finally` so a crash leaves no end event, and the decision to skip the agent `KEYFILE_LOAD` emit on `FileNotFoundError` (the raised exception is the signal). The generated edits were reviewed before integration, and the existing 15/15 test harness passes unchanged; a full closed-loop run was validated to produce a complete ordered event timeline with a collector attached.

4. **Commit `ff9e7111f561858355f0b64b0513c2a642f43bbf` and `8f53980` — Self-narrating demo, defense-first README, and setup script**  
   The demo (`DEMO.sh`), README rewrite, and `setup.sh` were produced under significant time pressure with an external-deadline deliverable needing to be shippable in an emergency. Pure time constraints motivated delegating this work to an LLM; the alternatives — hand-writing ~275 lines of styled bash plus a full documentation rewrite — could not be completed within the available window. The functional core (fixture, crypto, VM, detectors) remains manually authored as covered by the items above; this entry concerns only presentation/documentation/tooling work. The demo's behavioural claims were verified before committing (exit-0 closed-loop replay with live artifacts), and the README's statuses are intentionally conservative (tooling marked under construction). Commit `8f53980` (the presenter explainer added to `DEMO.sh`) is a small follow-up produced under the same constraints and reasoning, and is disclosed here for completeness.
