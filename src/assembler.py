...
#Calculates the byte size of a single instruction
def get_instruction_size(instruction):
    ...
    #Rejects single byte ops that incorrectly have an operand
...
#Resolves label names to their byte offsets in the assembled bytecode
def resolve_labels(assembly_lines):
    ...
    for line in assembly_lines:
        ...
        #Records label definitions at the current byte offset
...
#Encodes a single instruction line into bytecode bytes
...
#Assembles multi-line assembly text into a bytecode payload
def assemble_payload(bytecode):
    ...
    for line in lines:
        ...
        #Skips label definitions during assembly
        if cleaned_line.endswith(':'):
            continue
        ...
        #Resolves label references to relative offsets for jump instructions
